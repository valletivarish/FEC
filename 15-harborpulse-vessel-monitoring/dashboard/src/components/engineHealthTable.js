function buildRow(entry) {
  ...
  tr.innerHTML = `
    ...
    <td>${formatNumber(entry.engineRpm)}</td>
    <td>${formatNumber(entry.coolantTempC)}</td>
    <td>${formatNumber(entry.oilPressureKpa)}</td>
    <td>${formatNumber(entry.fuelFlowLph)}</td>
    <td>${formatNumber(entry.bearingWearEnergy, 3)}</td>
