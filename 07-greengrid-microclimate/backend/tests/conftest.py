...
os.environ.setdefault("GREENGRID_READINGS_TABLE", "GreenGridReadings")


def import_handler(function_name: str):
    """Every Lambda folder has its own handler.py; reimport fresh each time so moto state doesn't leak."""
    import importlib

    sys.modules.pop("handler", None)
    sys.modules.pop("shared.ddb", None)
    sys.modules.pop("shared.json_encoder", None)
